...
func SetUpInstance(ctx context.Context, client trillian.TrillianLogClient, cfg *configpb.HubConfig, opts InstanceOptions) (*PathHandlers, error) {
	...
	if len(cfg.SourceLog) == 0 {
		return nil, errors.New("need to specify SourceLog")
	}
	if cfg.PrivateKey == nil {
		return nil, errors.New("need to specify PrivateKey")
	}

	// Load the trusted Log public keys
	cryptoMap := make(map[string]logCryptoInfo)
	for _, srcLog := range cfg.SourceLog {
		if _, ok := cryptoMap[srcLog.Url]; ok {
			return nil, fmt.Errorf("Duplicate source log entry %s for URL %s", srcLog.Name, srcLog.Url)
		}
		pubKey, err := x509.ParsePKIXPublicKey(srcLog.PublicKey.Der)
		if err != nil {
			return nil, fmt.Errorf("Failed to parse public key for %s <%s>: %v", srcLog.Name, srcLog.Url, err)
		}
		var hasher crypto.Hash
		switch srcLog.HashAlgorithm {
		case sigpb.DigitallySigned_SHA256:
			hasher = crypto.SHA256
		default:
			return nil, fmt.Errorf("Failed to determine hash algorithm %d", srcLog.HashAlgorithm)
		}
		cryptoMap[srcLog.Url] = logCryptoInfo{pubKeyData: srcLog.PublicKey.Der, pubKey: pubKey, hasher: hasher}
	}
